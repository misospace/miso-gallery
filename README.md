...
| `UPLOAD_DIR` | No | `$DATA_FOLDER/input` | Where in-browser uploads land. The upload button on the gallery root saves images here (e.g. `root/input/*` when the share is mounted at `DATA_FOLDER=/root`) so ad-hoc images can be shared without write access to the image share (issue #485). |
...
The gallery root page shows an upload button (behind authentication) that saves selected images to `UPLOAD_DIR` (default `<DATA_FOLDER>/input`). This lets users share ad-hoc images through the gallery without write access to the image share (issue #485).
...
Releases are cut by the standing **release-please** PR: merge the open `release: X.Y.Z` PR, and release-please bumps `app.py`, `pyproject.toml`, and the manifest together, tags the merge commit with a plain-semver tag, and creates the GitHub release.

The **Release** workflow (triggered by a published release) validates that `APP_VERSION` in `app.py` matches the release tag before building Docker images. If they diverge, the build fails to prevent releasing a binary with an incorrect version string.
